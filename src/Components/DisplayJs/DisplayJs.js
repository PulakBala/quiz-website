...
const DisplayJs = ({js}) => {
    const {options,question,correctAnswer} = js;
    ...
    return (
        <div className='border w-9/12 my-8 bg-orange-100 py-10 rounded-lg'>
           ...
           <form className='grid grid-cols-2 place-content-center h-auto p-4 gap-10 mt-12'>
                ...
                <input onClick={()=> quizToast(options[0])} className=' h-12 w-4 mr-2' type="radio" id="html" name="fav_language"/> 
                {options[0]}</p>

                <p className='border border-red-900 p-4 flex items-center justify-center'>
                <input onClick={()=> quizToast(options[1])} className=' h-12 w-4 mr-2' type="radio" id="html" name="fav_language"/> 
                {options[1]}</p>

                <p className='border border-red-900 p-4 h-20 flex items-center justify-center'> 
                <input onClick={()=> quizToast(options[2])} className=' h-12 w-4 mr-2' type="radio" id="html" name="fav_language"/> 
                {options[2]}</p>

                <p className='border border-red-900 p-4 flex items-center justify-center'> 
                <input onClick={()=> quizToast(options[3])} className=' h-12 w-4 mr-2' type="radio" id="html" name="fav_language"/> 
                {options[3]}</p>
